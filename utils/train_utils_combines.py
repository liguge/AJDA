...
class train_utils(object):
    ...
    def setup(self):
        ...
        if args.distance_metric:
            ...
            elif args.distance_loss == "AJDA":
                self.softmax_layer = nn.Softmax(dim=1)
                self.distance_loss = AJDA
            else:
                        ...
                        if phase != 'source_train' or epoch < args.middle_epoch:
                            ...
                            if self.distance_loss is not None:
                                if args.distance_loss == 'MK-MMD':
                                    ...
                                    # softmax_out = self.softmax_layer(outputs)
                                    softmax_out = outputs
                                    pseudo_labels = F.softmax(outputs.narrow(0, labels.size(0), inputs.size(0)-labels.size(0)), dim=-1)
                                    pseudo_labels_onehot = torch.zeros_like(pseudo_labels)
                                    pseudo_labels_onehot.scatter_(1, torch.argmax(pseudo_labels, dim=1, keepdim=True),
                                                                  1)
                                    true_labels_onehot = F.one_hot(labels, num_classes=args.num_classes)
                                    _, _, distance_loss = self.distance_loss(softmax_out.narrow(0, 0, labels.size(0)), true_labels_onehot,
                                                                       softmax_out.narrow(0, labels.size(0),
                                                                                        inputs.size(0) - labels.size(
                                                                                            0)),
                                                                        pseudo_labels_onehot, thre=0.9, mult=1.5)
                                else:
                                    raise Exception("loss not implement")
